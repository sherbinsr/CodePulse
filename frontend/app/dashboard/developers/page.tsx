      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        {!loading && !error && org && devs.length === 0 && (
          <div className="flex flex-col items-center justify-center h-64 text-center gap-2">
            <p className="text-slate-500 dark:text-slate-400">No developer data found for <strong>{org}</strong>.</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700">
                    <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Developer</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">PRs</th>
                    ...
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Reviews</th>
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                  {devs.map((dev) => (
                    <tr
                      ...
                      className={`cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors ${selected?.login === dev.login ? "bg-indigo-50 dark:bg-indigo-950 hover:bg-indigo-50 dark:hover:bg-indigo-950" : ""}`}
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          {dev.avatar_url ? (
                            ...
                            <div className="w-10 h-10 rounded-full bg-indigo-50 dark:bg-indigo-950 flex items-center justify-center text-sm font-bold text-indigo-600 flex-shrink-0">
                              {dev.login[0].toUpperCase()}
                            </div>
                          )}
                          <span className="font-medium text-slate-800 dark:text-slate-200">{dev.login}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5 text-right text-slate-700 dark:text-slate-300">{dev.total_prs}</td>
                      <td className="px-4 py-3.5 text-right text-slate-700 dark:text-slate-300">{dev.merged_prs}</td>
                      ...
                      <td className="px-4 py-3.5 text-right text-slate-700 dark:text-slate-300">{dev.reviews_given}</td>
                      <td className="px-4 py-3.5 text-right text-slate-500 dark:text-slate-400">{formatHours(dev.avg_merge_hours)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="space-y-4">
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
                  <div className="flex items-center gap-4 mb-5">
                    {selected.avatar_url ? (
                      <Image src={selected.avatar_url} alt={selected.login} width={48} height={48} className="rounded-full ring-2 ring-slate-100 dark:ring-slate-700" />
                    ) : (
                      <div className="w-12 h-12 rounded-full bg-indigo-50 dark:bg-indigo-950 flex items-center justify-center text-xl font-bold text-indigo-600 ring-2 ring-indigo-100 dark:ring-indigo-900">
                        {selected.login[0].toUpperCase()}
                      </div>
                    )}
                    <div>
                      <h3 className="font-semibold text-slate-900 dark:text-slate-100">{selected.login}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <div key={label as string} className="rounded-xl bg-slate-50 dark:bg-slate-800 p-3">
                        ...
                        <p className="font-semibold text-slate-800 dark:text-slate-200 mt-1">{val}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
                  <h4 className="font-semibold text-slate-700 dark:text-slate-300 text-sm mb-4">Activity Radar</h4>
