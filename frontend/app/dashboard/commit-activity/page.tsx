...
function CommitContent() {
  ...
  // Pivot code churn: week → { week, repo1: count, repo2: count, ... }
      ...
      <div className="flex-1 p-6 overflow-auto space-y-6">
        ...
        {isEmpty && org && (
          <div className="flex flex-col items-center justify-center h-64 text-center gap-2">
            <p className="text-slate-500">No commit data found for <strong>{org}</strong>.</p>
            <p className="text-sm text-slate-400">Commit data is collected during sync (last 90 days).</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100">
                <h3 className="font-semibold text-slate-800">Developer Commit Activity</h3>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Developer</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Commits</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Active Days</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Commits/Day</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">After-Hours</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Weekend</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Repos</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {activity.map((dev) => (
                    <tr key={dev.author_login} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-slate-800">{dev.author_login}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5 text-right font-medium text-slate-800">{dev.total_commits}</td>
                      <td className="px-4 py-3.5 text-right text-slate-500">{dev.active_days}</td>
                      <td className="px-4 py-3.5 text-right text-slate-500">{dev.commits_per_active_day}</td>
                      ...
                      <td className="px-4 py-3.5 text-right text-slate-600">{dev.repos_contributed}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {churnChart.weeks.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <h3 className="font-semibold text-slate-800 mb-1">Weekly Code Churn</h3>
            ...
            {activity.some((d) => d.after_hours_pct >= 15) && (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <h3 className="font-semibold text-slate-800 mb-1">After-Hours Signal</h3>
                  ...
                  {activity
                    ...
                    .map((dev) => (
                      <div key={dev.author_login} className="flex items-center gap-4">
                        ...
                        <span className="font-medium text-slate-800 text-sm w-32 truncate">{dev.author_login}</span>
