...
function GrantPermissionBanner({ onCheckAgain }: { onCheckAgain?: () => void }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-amber-200 p-8 text-center max-w-xl mx-auto mt-16">
      ...
      <div className="flex flex-col gap-3">
        <button
          ...
          className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-lg font-medium transition-colors"
        >
        ...
        {onCheckAgain && (
          <button
            ...
            className="flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-6 py-3 rounded-lg font-medium transition-colors text-sm"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {orgs.map((org) => (
          <button
            ...
            className="flex items-center gap-4 bg-white border border-slate-200 hover:border-indigo-300 hover:shadow-md rounded-xl p-5 text-left transition-all group"
          >
            {org.avatar_url ? (
              <Image
                src={org.avatar_url}
      ...
      <div className="flex-1 p-6 space-y-6 overflow-auto">
        ...
        {!loading && !error && (!overview || overview.total_prs === 0) && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-amber-800">
            No data yet for <strong>{org}</strong>. Click <strong>Sync Now</strong> in the header to fetch PR data from GitHub.
          </div>
        )}
